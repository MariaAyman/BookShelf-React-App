  ...
  return (
    <>
      <div className="input-group mb-3" style={{width: 500}}>
        <input type="text" className="form-control" placeholder="Search Books ..." aria-label="Recipient's username" aria-describedby="button-addon2" value={searchText} onChange={e => handleSearch(e.target.value, ['title', 'author', 'series'])} onFocus={showAutoSuggest} onBlur={hideAutoSuggest} />
      ...
      <ul className="search-suggestions" style={{listStyle: 'none'}}>
        ...
        {!display && matched.map(m =>(
          <Link key={m.id} to={`/book/${m.id}`} style={{textDecoration: 'none', color: '#0dcaf0'}} className="autosuggest-link">
